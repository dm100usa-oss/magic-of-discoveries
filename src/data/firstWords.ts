...
export const wordsSteps: Record<UiLang, WordsStep[]> = {
  ...
  ru: [
    ...
    },
    {
      n: "3",
      title: "Покажи слово",
      text: "Слово написано под рисунком контурными буквами, его тоже можно раскрасить. Покажите пальцем, прочитайте вслух и идите дальше.",
    },
  ],
};
...
export const wordsHub: Partial<Record<UiLang, WordsHubCopy>> = {
  ...
  ru: {
    ...
    definition:
      "Раскраска с первыми словами - это страница, где нарисован один знакомый предмет, а под ним напечатано его название. Ребенок раскрашивает, взрослый называет. Слово набрано контурными буквами, поэтому его тоже можно раскрасить. На листе нет фона и второго предмета, который отвлекал бы внимание.",
    showcase:
      "Три настоящие страницы - ровно такие, как они напечатаны в книгах: рисунок и слово под ним контурными буквами.",
    ...
    faq: [
      {
        ...
        a: "Это страница, где нарисован один знакомый предмет, а под ним напечатано его название. Ребенок раскрашивает, взрослый называет предмет и показывает слово. Само слово набрано контурными буквами, поэтому его тоже можно раскрасить.",
      },
      {
...
const RU_PAGES: Record<string, RuWordsPage> = {
  ...
  food: {
    ...
    copy: {
      ...
      definition:
        "Первые слова о еде - это названия того, что ребенок видит каждый день: яблоко, банан, морковь, хлеб, торт. Такие слова легче входят в речь, потому что предмет можно потрогать и попробовать. На странице раскраски продукт нарисован крупно и просто, а его название стоит под рисунком контурными буквами.",
  ...
  nature: {
    ...
    copy: {
      ...
      definition:
        "Первые слова о природе - это названия того, что ребенок видит на прогулке и из окна: солнце, дерево, цветок, листок, облако. Такие слова удобно называть по дороге, потому что предметы постоянно перед глазами. В книге каждый нарисован крупно и просто, с названием под рисунком контурными буквами.",
  ...
  home: {
    ...
    copy: {
      ...
      definition:
        "Первые слова о доме - это названия вещей, которые окружают ребенка в комнате и на кухне: кружка, ложка, вилка, лампа, подушка. Их удобно называть по ходу дела, не устраивая специальных занятий. В книге каждая вещь нарисована отдельно, крупно, с названием под рисунком контурными буквами.",
