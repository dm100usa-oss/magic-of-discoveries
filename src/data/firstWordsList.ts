...
export type WordPair = { en: string; es: string; ru?: string };
...
/** Все слова темы, по книгам. Пустые книги отбрасываем.
    Русские издания хранятся отдельным списком: там другие книги. */
export function themeWords(
  ...
  if (lang === "ru") {
    return wordBooks
      .map((book) => ({
        book,
        words: (WORDS_RU[book][theme] ?? []).map((w) => ({
          en: w,
          es: w,
          ru: w,
        })),
...
export const themeCount = (theme: WordTheme, lang: UiLang = "en"): number =>
  wordBooks.reduce(
    (n, b) =>
      n + ((lang === "ru" ? WORDS_RU[b][theme] : WORDS[b][theme])?.length ?? 0),
    0,
  );

/** Слово на языке страницы. */
export const wordText = (p: WordPair, lang: UiLang): string =>
  lang === "ru" ? (p.ru ?? p.en) : lang === "es" ? p.es : p.en;

/* ------------------------------------------------------------------ */
/*  Русские издания                                                    */
/*                                                                     */
/*  Слова прочитаны со страниц русских книг, по одной странице за раз, */
/*  и записаны ровно так, как они там напечатаны. Где в книге опечатка,*/
/*  она сохранена: страница сайта обязана совпадать с тем, что получит */
/*  покупатель. Исправлять такие слова надо в новом издании книги.     */
/*                                                                     */
/*  Первая книга-раскраска на русском это та же книга, что английская  */
/*  и испанская: те же рисунки в том же порядке. Две книжки Максима    */
/*  Макса на русском собраны из своих рисунков, поэтому их списки      */
/*  здесь отдельные и с английскими не совпадают.                      */
/* ------------------------------------------------------------------ */

export const WORDS_RU: Record<WordBook, Partial<Record<WordTheme, string[]>>> = {
  "first-coloring-book-111": {
    animals: [
      "Лев", "Слон", "Зебра", "Попугай", "Крокодил", "Обезьяна", "Кенгуру",
      "Носорог", "Фламинго", "Лемур", "Колибри", "Жираф", "Коала", "Лягушка",
      "Альпака", "Кролик", "Сова", "Ёж", "Коза", "Летучая мышь", "Енот",
      "Медведь", "Лиса", "Курица", "Хамелеон", "Корова", "Бобр", "Орёл",
      "Хомяк", "Кошка", "Собака", "Белка", "Утка", "Олень", "Мышь", "Пчела",
      "Стрекоза", "Улитка", "Бабочка", "Акула", "Дельфин", "Кит", "Краб",
      "Осьминог", "Медуза", "Морская черепаха", "Рыба-ангел", "Морской конёк",
      "Тюлень", "Рыба-клоун", "Моллюск", "Аксолотль", "Рыба-шар", "Креветка",
      "Скат",
    ],
    fairy: [
      "Русалка", "Единорог", "Дракон", "Корона", "Гном", "Грифон", "Тролль",
      "Фея", "Волшебный котел", "Шляпа волшебника", "Волшебное зелье",
    ],
    vehicles: [
      "Машина", "Вертолет", "Самолет", "Воздушный шар", "Корабль",
      "Подводная лодка", "Ракета", "Скутер",
    ],
    objects: [
      "Скейтборд", "Воздушный змей", "Бадминтон", "Американский футбол",
      "Камера", "Барабан", "Пляжный мяч", "Очки", "Пляжный зонт", "Шляпа",
      "Глобус", "Подарок", "Геймпад",
    ],
    nature: [
      "Кленовый лист", "Роза", "Гриб", "Клевер", "Подсолнух", "Шишка",
      "Кактус", "Ландыш", "Лотос", "Тюльпан",
    ],
    food: [
  ...
  "little-max-coloring-1": {
    animals: [
      "Енот", "Гусыня", "Гусенок", "Улитка", "Кошка", "Овечка", "Собака",
      "Заяц", "Курица", "Цыпленок", "Белка", "Ослик", "Лиса", "Поросёнок",
      "Корова", "Бабочка", "Стрекоза", "Гусеница", "Шмель", "Божья коровка",
      "Лягушка", "Птица",
    ],
    nature: [
      "Дерево", "Подсолнух", "Растение в горшке", "Цветок", "Ваза с цветами",
      "Ёлка", "Лотос", "Гриб", "Листок", "Облако", "Солнце", "Радуга",
      "Снежинка", "Луна", "Звезда",
    ],
    food: [
      "Помидор", "Морковь", "Огурец", "Капуста", "Яблоко", "Апельсин", "Банан",
      "Вишня", "Малина", "Клубника", "Мороженное", "Тосты", "Яичница",
      "Печенье", "Пирог", "Пончик", "Торт", "Пирожное", "Лимонад", "Конфеты",
      "Варенье", "Сыр",
    ],
    vehicles: ["Машина", "Ракета", "Автобус"],
    toys: [
      "Вертушка", "Воздушный змей", "Пазлы", "Единорог", "Медведь",
      "Пирамидка", "Кораблик", "Кубики", "Ведро с лопаткой", "Мяч",
    ],
    clothes: [
      "Шорты", "Юбка", "Варежки", "Носки", "Футболка", "Шапка", "Платье",
      "Панама", "Кеды", "Фуражка", "Очки", "Бант",
    ],
    home: [
      "Дом", "Кружка", "Кувшин", "Вилка", "Ложка", "Расчёска", "Лампа",
      "Будильник", "Нитки", "Ножницы", "Пуговицы", "Иголка с ниткой",
      "Клубок шерсти", "Подушка",
    ],
    objects: [
      "Подарок", "Карандаши", "Зонт", "Забор", "Скамейка", "Мельница", "Маяк",
      "Фонарик", "Фонарь", "Книга", "Конверт", "Телефон", "Перо",
    ],
  },

  "little-max-coloring-2": {
    animals: [
      "Лев", "Слоненок", "Панда", "Зебра", "Жираф", "Обезьяна", "Пони",
      "Попугай", "Фламинго", "Сова", "Павлин", "Тукан", "Черепаха", "Рыбка",
      "Дельфин", "Краб", "Кит", "Осьминог", "Медуза", "Морской конек",
      "Моллюск", "Креветка", "Морская звезда",
    ],
    nature: [
      "Пальма", "Лист", "Желудь", "Камыш", "Бамбук", "Монстера", "Кактус",
      "Ракушка", "Водоросли", "Кораллы", "Кристаллы", "Горы", "Волна", "Гриб",
      "Лесной орех", "Ландыш", "Ирис", "Орхидея", "Лилия",
    ],
    food: [
      "Перец", "Брокколи", "Тыква", "Лук", "Чеснок", "Кукуруза", "Гранат",
      "Киви", "Груша", "Ананас", "Авокадо", "Арбуз", "Дыня", "Виноград",
      "Кофе", "Сэндвич", "Бургер", "Пицца", "Хлеб", "Шоколад", "Попкорн",
      "Сладкая вата", "Леденец", "Мороженое", "Мед", "Яблоко в карамели",
      "Сок",
    ],
    vehicles: [
      "Машина", "Лодка", "Вертолет", "Воздушный шар", "Подводная лодка",
    ],
    objects: [
      "Скейтборд", "Мяч регби", "Футбольный мяч", "Пляжный мяч",
      "Маска для плавания", "Плавательный круг", "Доска для серфинга",
      "Удочка", "Фотоаппарат", "Микрофон", "Барабан", "Труба", "Маракасы",
      "Бубен", "Колонки музыкальные", "Портфель", "Якорь", "Чемодан",
      "Компас", "Бочка",
    ],
    clothes: [
      "Кепка", "Цилиндр", "Сомбреро", "Сандали", "Сланцы", "Майка",
    ],
    toys: [
      "Воздушные шары", "Флажок", "Сачек", "Мыльные пузыри", "Качель",
    ],
    home: [
      "Кастрюля", "Сковорода", "Чайник", "Корзина", "Гирлянда из флажков",
      "Свеча",
    ],
  },
};
