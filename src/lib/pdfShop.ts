...
/* Цена файла зависит от вида книги.

   Раскраска стоит 3.99, книга рисования и история 4.99. Причина простая:
   раскраску родитель сравнивает с бумажной книгой на Amazon за 6.99 и с
   бесплатными листами, которые лежат везде, поэтому цена должна быть
   заметно ниже. Книга рисования и история дают больше, чем один лист,
   и держат прежнюю цену.

   Раньше цена была одна на весь каталог, и на toddlercoloringbook.com
   та же самая раскраска продавалась за 3.99, а здесь за 4.99. Одна книга
   с двумя ценами на двух наших сайтах подрывает доверие сильнее, чем
   разница в доллар. */
...
export function pdfPriceLabel(id: string): string {
  return "$" + (pdfPriceCents(id) / 100).toFixed(2);
}
